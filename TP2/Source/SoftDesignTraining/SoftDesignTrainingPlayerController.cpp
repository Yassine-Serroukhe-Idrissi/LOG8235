...
void ASoftDesignTrainingPlayerController::MoveCharacter()
{
    ...
    if (UNavigationPath* path = navSystem->FindPathToLocationSynchronously(this, pawn->GetActorLocation(), target.Location))
    {
        ...
        {
            DrawDebugLine(GetWorld(), points[i - 1] + FVector(0, 0, 10), points[i] + FVector(0, 0, 10), FColor::Blue, true, -1.f, 0, 5.f);
            DrawDebugSphere(GetWorld(), points[i] + FVector(0, 0, 10), 20.f, 4, FColor::Blue, true);
        }
...
void ASoftDesignTrainingPlayerController::Activate()
{
    ...
    m_PathFollowingComponent->AbortMove(*this, FPathFollowingResultFlags::ForcedScript | FPathFollowingResultFlags::NewRequest, FAIRequestID::CurrentRequest, EPathFollowingVelocityMode::Keep);
    ClearPathDebug();
